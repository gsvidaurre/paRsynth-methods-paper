...
n_individuals <- 5
n_calls <- 5
string_length <- (group_information + individual_information) + 20
...
# Why are there only 10 unique strings...
length(unique(calls$Call))

# There's only one unique string per individual here...
View(calls[which(!duplicated(calls$Call)), ])
...
# frequencies <- as.numeric(df[1, grep("^Frequency", colnames(df))])
# 
