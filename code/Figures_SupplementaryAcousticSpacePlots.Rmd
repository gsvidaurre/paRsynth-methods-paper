...
TKTK update figures such that main figures only include some groups and the supplement includes everything.
SPCC group membership group polygons has bad legend...

TKTK choose groups 2 and 3 for the main acoustic space plots for the group membership dataset.
TKTK choose groups 1 and 2 for the main acoustic space plots for the individual identity dataset.
Brittany will choose which spectrograms to include in the call catalogues for both datasets (2 individuals per group and 2-3 calls per individual).
...
# This is the image size that will be written to create a legend for the composite figure created in other software
ggsave(filename = file.path(figures_path, "AcousticSpace_SPCC_GroupMembership_IndividualPolygons_Legend.tiff"), width = 8, height = 4, units = "in", dpi = 300)
...
# Notes on supplementary figures

These different image files shoud be used to generate 2 supplementary figures: One showing patterns of variation in acoustic space for the group membership dataset across (dis)similarity methods and polygons at either social level, and a similar figure for the individual identity dataset.

Supplementary figure plans (one for the group membership dataset, one for the individual identity dataset): 
- Panel A should be edit distance, group-level polygons
- Panel B should be SPCC, group-level polygons
- Panel C should be edit distance, individual-level polygons
- Panel D should be SPCC, individual-level polygons
