...
# i <- 1
information <- data.table::rbindlist(pblapply(1:length(N), function(j){
...
# i <- 2 # information encoding treatments
    ...
    lapply(1:length(group_information), function(i){
      ...
      res_tmp <- data.table::rbindlist(lapply(1:iteratns, function(n){
        ...
        if(zeroes > 0){
          ...
          # Then set the values of 1 off of the diagonal (in both the upper and lower triangle) to the next highest similarity value in this matrix
          edit_mat2 <- edit_mat
          edit_mat2[lower.tri(edit_mat2)][edit_mat2[lower.tri(edit_mat2)] == 0] <- round(sec_min_val/10, 8)
          edit_mat2[upper.tri(edit_mat2)][edit_mat2[upper.tri(edit_mat2)] == 0] <- round(sec_min_val/10, 8)
          
          edit_mat[lower.tri(edit_mat)][edit_mat[lower.tri(edit_mat)] == 0] <- 1
          edit_mat[upper.tri(edit_mat)][edit_mat[upper.tri(edit_mat)] == 0] <- 1
          
          valid_notes <- "Validating zeroes off of the diagonal"
        ...
        dimnames(edit_mat2) <- list(dimnames(xc_mat)[[1]], dimnames(xc_mat)[[2]])
        ...
        edit_dist_mat2 <- stats::as.dist(edit_mat2, diag = TRUE, upper = TRUE)
        ...
        iso_edit2 <- invisible(MASS::isoMDS(edit_dist_mat2, k = k, maxit = 1000, trace = FALSE))
        ...
        mds_edit2 <- data.frame(sound.files = dimnames(iso_edit2$points)[[1]]) %>%
          dplyr::mutate(
            X = iso_edit2$points[, 1],
            Y = iso_edit2$points[, 2]
          ) %>%
          inner_join(
            sel_tbl %>%
              dplyr::select(sound.files, group_ID, individual_ID, call_ID),
        ...
        # Validating against setting zeroes to 1s
        mds_edit_filt2 <- mds_edit2[grep(paste(grp_calls, collapse = "|"), mds_edit2[["sound.files"]]), ]
        
        if(nrow(mds_edit_filt2) != N[z] * length(unique(mds_edit_filt2[["group_ID"]]))){
          stop("The number of call exemplars per class must match between social levels")
        }
        
        edit_km_grp2 <- kmeans(x = mds_edit_filt2[, grep("X|Y", names(mds_edit_filt2))], centers = num_clusters)
        # str(edit_km_grp2)
        
        # Combine the resulting cluster labels with the original information about signals
        # We can do this column addition because we used mds_spcc directly as input to the clustering algorithm, so rows in each object are in the same order
        mds_edit_clust2 <- mds_edit_filt2 %>% 
          dplyr::mutate(
            inferred_label = edit_km_grp2$cluster
          ) %>% 
          dplyr::rename(
            `true_label` = "group_ID"
          ) %>% 
        ...
        # glimpse(mds_edit_clust2)
        
        # Calculate accuracy scores after bipartite matching of original class and cluster labels
        acc_edit2 <- find_best_mapping(mds_edit_clust2)
        acc_edit_grp2 <- acc_edit2[["maximum_accuracy"]]
        ...
        acc_edit_inds2 <- unlist(lapply(1:N[z], function(m){
          
          tmp <- mds_edit2[mds_edit2$group_ID == m, ]
        ...
        # Take the mean of the individual-level accuracies across groups
        acc_edit_ind2 <- mean(acc_edit_inds2)
        ...
        results <- data.frame(
          ...
          accuracy_edit_group2 = acc_edit_grp2,
          accuracy_mean_SPCC_individual = acc_spcc_ind,
          accuracy_mean_edit_individual = acc_edit_ind,
          accuracy_mean_edit_individual2 = acc_edit_ind2,
          notes = paste(neg_notes, one_notes, zero_notes, valid_notes, sep = "; ")
        )
      ...
      res_tmp %>% 
        write.csv(file.path(data_path, paste("Validation_Accuracy_", paste(paste("classes", N[z], sep = "-"), paste("groupInfo", group_information[i], sep = "-"), sep = "_"), ".csv", sep = "")), row.names = FALSE)
...
files <- list.files(path = data_path, pattern = "Validation_Accuracy_")
files
...
# Next, check how the edit distance results compare when replacing zeroes off of the diagonal with 0.1 versus 1 (suffix of 2)
glimpse(res)

# Positive differences (expressed as a percentage) mean that the edit distance modification of zeroes off of the diagonal to 0.1 led to higher accuracy. The reverse is true for negative values, such that the modification to convert zeroes off of the diagonal to 0.1 led to lower accuracy.
res %>% 
  dplyr::mutate(diff_perc = (accuracy_edit_group - accuracy_edit_group2 ) * 100) %>% 
  dplyr::select(n_groups, group_information_proportion, diff_perc)

# The biggest differences here are in group-level accuracy, but these results favor the solution that replaces zeroes off of the diagonal with 0.1 instead of 1 (higher accuracies on average for that solution). See below for a similar trend with individual-level accuracy. On average, replacing zeroes off of the diagonal with 0.1 instead of 1 yields 2% higher accuracy for group-level accuracy, and 0.32% higher accuracy at the individual-level. Even the largest difference in accuracy between ways of handling these edit distance values off of the diagonal favors using a value of 0.1 instead of 1 (27% greater accuracy when using 0.1).

# n_groups group_information_proportion diff_perc
# <int>                        <num>     <num>
#   1:        2                    0.0000000  0.000000
# 2:        2                    0.3333333  0.000000
# 3:        2                    0.6666667  0.000000
# 4:        2                    1.0000000  0.000000
# 5:        4                    0.0000000  6.250000
# 6:        4                    0.3333333 -6.250000
# 7:        4                    0.6666667  6.250000
# 8:        4                    1.0000000  0.000000
# 9:        6                    0.0000000 -2.777778
# 10:        6                    0.3333333  5.555556
# 11:        6                    0.6666667  2.777778
# 12:        6                    1.0000000 27.777778
# 13:        8                    0.0000000  1.562500
# 14:        8                    0.3333333  1.562500
# 15:        8                    0.6666667 -6.250000
# 16:        8                    1.0000000 -3.125000
...
# min     mean      max
# 1 -6.25 2.083333 27.77778

res %>% 
  dplyr::mutate(diff_perc = (accuracy_mean_edit_individual - accuracy_mean_edit_individual2) * 100) %>% 
  dplyr::select(n_groups, group_information_proportion, diff_perc)

# n_groups group_information_proportion  diff_perc
# <int>                        <num>      <num>
#   1:        2                    0.0000000  0.0000000
# 2:        2                    0.3333333  0.0000000
# 3:        2                    0.6666667  0.0000000
# 4:        2                    1.0000000  0.0000000
# 5:        4                    0.0000000 -3.1250000
# 6:        4                    0.3333333 -3.1250000
# 7:        4                    0.6666667  1.5625000
# 8:        4                    1.0000000  7.8125000
# 9:        6                    0.0000000  6.0185185
# 10:        6                    0.3333333 -2.7777778
# 11:        6                    0.6666667  3.7037037
# 12:        6                    1.0000000 -1.8518519
# 13:        8                    0.0000000  0.5859375
# 14:        8                    0.3333333 -0.1953125
# 15:        8                    0.6666667 -3.3203125
# 16:        8                    1.0000000 -0.1953125
...
# min     mean    max
# 1 -3.320312 0.318287 7.8125

# Write out validation results
res %>% 
  write.csv(file.path(data_path, "Validation_edit_distance_handling.csv"), row.names = FALSE)
