---
title: "Generate spectrogram image files for example datasets"
...
We need to select files that match the groups chosen for the acoustic space plots. 

For the Group Information Dataset, we need to choose 2 calls from 2 individuals from Group 2 and Group 3
...
For the Individual Information Dataset, we need to to choose 2 calls from 2 individuals from Group 1 and Group 2

Group 1: Individual 1 (calls 2 and 3), Individual 3 (calls 1 and 2)
Group 2: Individual 1 (calls 3 and 5), Indivodual 2 (calls 1 and 2)

Make sure to move the selected files to the "lexicon" folder before proceeding with this code. 
...
invisible(lapply(X = 1:length(lexicon_wavs), FUN = function(w){
  ...
  ggsave(filename = file.path(figures_path, gsub(pattern = ".wav$", replace = ".jpeg", lexicon_wavs[w])), units = "in", width = 1.4, height = 1.4, dpi = 600)
}))
