...
invisible(pblapply(1:length(img_list), function(i){
  ...
  lapply(1:length(img_list[[i]]), function(z){
    ...
    # saving thick versions of the figures for presentations
    magick::image_write(magick::image_border(image = img, color = cols_indivs[i], geometry = "20x20"), path = file.path(git_path, "Fig_4",  paste(img_list[[i]][z], "_border_thick.jpeg", sep = "")), format = "jpeg")
  })
