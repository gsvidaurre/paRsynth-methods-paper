...
# TKTK check that values of 0.1 don't influence downstream computations with the matrix. Check against setting these zeroes to 1...then check with Vanessa

# Get the minimum non-zero similarity value in the matrix, which should be 1
...
# Step 3: Reduce the dimensionality of the SPCC and edit distance matrices with multidimensional scaling.
```{r}
...
TKTK split these plots into panels by dataset

Create a plot of both datasets with group-level polygons in SPCC acoustic space, this will be a supplementary figure.
```{r}
...
cols_grp <- cols[1:length(levels(mds_df_all$group))]
...
TKTK for continuing to update plots, consider the following: Add the edit distance as a panel to each main figure. So each main figure will be a lexicon for the given dataset (Group membership or Inidvidual identity), then there will be 2 acoustic space plots on the right: one generated using the edit distance and the other generated using SPCC. Consider dropping the individual-level polygons for group membership plots and vice versa, or keeping these in the supplement.
...
# This is the image size that will be written to create alegend for the composite figure created in other software
ggsave(filename = file.path(figures_path, "AcousticSpace_GroupMembership_GroupPolygons_Legend.tiff"), width = 8, height = 4, units = "in", dpi = 300)
