...
hulls <- plyr::ddply(mds_df_all_spcc, "dataset_group", function(x){
  x[chull(x$X, x$Y), ]
})
...
Create plots of both datasets with group and individual-level polygons in SPCC and edit distance acoustic space. These will be supplementary figures: one per method with the group-level polygons figure on the top and individual-level polygons figure on the bottom.

### Supplement: SPCC, both datasets, group-level polygons
...
mds_df_all_spcc %>% 
  ...
  scale_x_continuous(limits = c(-0.3, 0.4), breaks = seq(-0.3, 0.4, 0.1), labels = sprintf("%.2f", round(seq(-0.3, 0.4, 0.1), 2))) +
...
ggsave(filename = file.path(figures_path, "AcousticSpace_SPCC_bothDatasets_GroupPolygons.tiff"), width = 6, height = 4, units = "in", dpi = 300)

```

### Supplement: edit distance, both datasets, group-level polygons

The same plot, but the edit distance.
```{r}

mds_df_all_edit <- mds_edit %>%
  # Create a column that combines dataset and group
  dplyr::mutate(
    dataset_group = paste(dataset, group_ID, sep = "-"),
    dataset_group = factor(dataset_group)
...
hulls <- plyr::ddply(mds_df_all_edit, "dataset_group", function(x){
  x[chull(x$X, x$Y), ]
})
...
mds_df_all_edit %>% 
  ...
  # Specify the x and y-axis limits so that these are the same across plots
  # Also make sure that each label retains two decimal places even when there is a trailing zero
  scale_x_continuous(limits = c(-5, 4.5), breaks = seq(-5, 4.5, 1), labels = sprintf("%.2f", round(seq(-5, 4.5, 1), 2))) +
  scale_y_continuous(limits = c(-4.5, 4), breaks = seq(-4.5, 4, 1), labels = sprintf("%.2f", round(seq(-4.5, 4, 1), 2))) +
  # facet_wrap(~ dataset) +
  theme_bw() +
  theme(
    axis.title = element_text(size = 14),
...
# Save this plot as an image file
ggsave(filename = file.path(figures_path, "AcousticSpace_edit_bothDatasets_GroupPolygons.tiff"), width = 6, height = 4, units = "in", dpi = 300)

```

### Supplement: individual-level polygons

TKTK fix these plots
...
cols <- cols[1:length(levels(mds_df_all_spcc$group))]
cols

# Fills by unique individual
levels(mds_df_all_spcc$unique_individuals)
fills <- rep(cols, each = length(levels(mds_df_all_spcc$individual_ID)))

# Shapes by individual within groups
shps <- rep(c(7, 8, 10, 4, 16, 17, 21, 23, 24, 6), length(levels(mds_df_all_spcc$individual_ID)))

```

### Supplement: SPCC, both datasets, individual-level polygons

```{r}

hulls <- plyr::ddply(mds_df_all_spcc, "unique_individuals", function(x){
  x[chull(x$X, x$Y), ]
})
...
mds_df_all_spcc %>% 
  ggplot(aes(x = X, y = Y)) + 
  geom_polygon(data = hulls, aes(x = X, y = Y, fill = dataset_group), alpha = 0.2, linewidth = 0.2, show.legend = FALSE) +
  geom_point(aes(color = unique_individuals, shape = unique_individuals), size = 2, stroke = 0.5) +
  scale_shape_manual(values = shps) +
  scale_color_manual(values = cols) +
  scale_fill_manual(values = cols) +
  xlab("") + ylab("") +  
