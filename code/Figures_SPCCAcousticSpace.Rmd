...
Calls from both datasets in the same universal acoustic space.
```{r}

levels(mds_df$dataset)
...
mds_df %>%
  ggplot(aes(x = X, y = Y, color = dataset, fill = dataset, shape = dataset)) + 
...
shps <- rep(c(0, 1, 2, 4, 16, 17, 21, 23, 24, 6), length(levels(mds_df$group_ID)))
...
# Colors by individual within groups (10)
cols <- scales::alpha(c("navy", "orange", "forestgreen", "firebrick", "grey", "black", "purple", "khaki", "pink", "limegreen"), 0.65)

# Shapes by individual within groups
shps <- rep(c(0, 1, 2, 4, 16, 17, 21, 23, 24, 6), length(levels(mds_df$group_ID)))

# Convex hull polygons per unique individual
...
mds_df %>%
  # Filter by the individual identity calls
  dplyr::filter(
