---
title: <center style="font-size:30px;font-style:normal;color:#0E0E7D;">Simulation Experiments for the Supercomputer</center>
author: |
  <br />
...
**Purpose**: This version of the Simulation Experiments code is designed to be run on MSU's supercomputer in order to increase the number of iterations per treatment, and contains different pathing structures compared to the original Simulation Experiments code that can be run on personal computers. The code creating the audio files and performing the analysis pipeline has not been changed. This code is different form the main supercomputer code in that it allows for running specific treatment combinations by specifying 1 number of classes option and 1 group/individual information option. To allow for this flexibility, this code also has options for calculating group and individual information proportions that work in these single treatment runs. These changes were necessary to efficiently run classes 6 and 8 conditions on the supercomputer after runs of all treatment combinations failed sue to running out of time or memory in initial work submissions. The AccurayPlots_EffectSize code reads in the data files generated on the supercomputer and performs checks to ensure that proper treatment combinations, with proper calculations of gorup and individual information proportion were run.

We designed simulation experiments to test the detection of identity information in synthetic vocalizations with spectrographic cross-correlation, a computational tool traditionally applied to biacoustics datasets. In these synthetic experiments, we manipulated the complexity of identity information encoding (e.g. identity information was encoded over one or two social levels), as well as the density of the simulated signaling environment (the number of classes at each social level, or the size of synthetic datasets).
...
# Set cores for parallel processing, save 2 cores for overhead. Originally allocating 20 cores on the supercomputer to this task
...
# Initialize a base path for the directory of this project on the supercomputer

path <- "/mnt/research/vocal-id-signals/paRsynth_methods_paper" 
data_path <- "/mnt/research/vocal-id-signals/paRsynth_methods_paper/data" 
...
source(file.path(path, "accuracy_score.R"))
...
Source the paRsynth functions directly from the project directory on the supercomputer. 
```{r}

###################### Manually update in every new coding session ##################################
paRsynth_path <- "/mnt/research/vocal-id-signals/paRsynth_methods_paper/paRsynth_functions"

# Get all of the R files for each function in this directory
function_files <- list.files(paRsynth_path, pattern = ".R$", full.names = TRUE)
function_files
...
######
# Specify batch number, for use when running on the super computer. This number will be used in the title of the output file.
batch <- "Batch1"
######
...
# to run single treatment combinations, comment out line above and uncomment specific individual_information treatment condition below
# individual_information <- 0
# individual_information <- 2
individual_information <- 4
# individual_information <- 6

# group_information <- rev(individual_information)
# to run single treatment combinations, comment out line above and uncomment specific group_information treatment condition below
# group_information <- 6
# group_information <- 4
group_information <- 2
# group_information <- 0
...
N <- c(2, 4, 6, 8)
# to run single treatment combinations, comment out line above and uncomment specific condition below 
# N <- 6
# N <- 8
...
experimental_treatements <- data.table::rbindlist(pblapply(1:length(N), function(j){
  ...
  tmp2 <- data.table::rbindlist(lapply(1:length(individual_information), function(i){
    ...
    tmp <- data.frame(
      ...
      # need to recalculate these proportions because they rely on the original vector used to generate information proportion conditions, and for some runs on the supercomputer, we used specific values instead of vectors to run iterations in batches for specific conditions. In this case, comment out the equations above and uncomment the equations below
      group_information_proportion = round(group_information[i] / (group_information[i] + individual_information[i]), 2),
      individual_information_proportion = round(individual_information[i] / (group_information[i] + individual_information[i]), 2)
    )
...
#######
# Initialize the number of iterations for each combination of information encoding and social environment treatments (15 total treatments). Should be 100 per batch
iteratns <- 1
#######
    ...
    lapply(1:length(group_information), function(i){
      ...
      res_tmp <- data.table::rbindlist(lapply(1:iteratns, function(n){
        ...
        results <- data.frame(
          ...
          # need to recalculate these proportions because they rely on the original vector used to generate information proportion conditions, and for some runs on the supercomputer, we used specific values instead of vectors to run iterations in batches for specific conditions. In this case, comment out the equations above and uncomment the equations below
          group_information_proportion = group_information[i] / (group_information[i] + individual_information[i]),
          individual_information_proportion = individual_information[i] / (group_information[i] + individual_information[i]),
      ...
      res_tmp %>% 
        write.csv(file.path(data_path, paste("Accuracy_", paste(paste("classes", N[z], sep = "-"), paste("groupInfo", group_information[i], sep = "-"), paste(batch), sep = "_"), ".csv", sep = "")), row.names = FALSE)
