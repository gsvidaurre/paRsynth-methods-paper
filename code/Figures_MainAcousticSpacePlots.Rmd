...
ggsave(filename = file.path(figures_path, "AcousticSpace_edit_GroupMembership_GroupPolygons_Legend.tiff"), width = 8, height = 4, units = "in", dpi = 300)
...
mds_spcc_grp %>%
  ...
  scale_fill_manual(values = cols_grp) +
  xlab("") + ylab("") + 
  guides(color = guide_legend(title = "Group", override.aes = list(size = 3), order = 1), shape = guide_legend(title = "Individual", nrow = 1), fill = guide_legend(title = "Group")) +
  # Uncomment this line to turn off the legend
  # guides(color = "none", shape = "none", fill = "none") +
  # Specify the x and y-axis limits so that these are the same across plots
  # Also make sure that each label retains two decimal places even when there is a trailing zero
...
# This is the image size that will be written to create a legend for the composite figure created in other software
ggsave(filename = file.path(figures_path, "AcousticSpace_SPCC_GroupMembership_GroupPolygons_Legend.tiff"), width = 8, height = 4, units = "in", dpi = 300)
