...
`./simulation_experiments`:
  ...
  1. `Simulation_Experiments.Rmd`: We carried out simulation experiments to address how well the original identiy information encoded at two social levels can be detected using computational pipelines that rely on distance measurements with SPCC (audio) and the edit distance (character strings). We then used multidimensional scaling features generated per distance method to cluster calls back to their original classes at each social level. These experiments were performed over different information encoding treatments at the group and individual levels and signaling environment densities (numbers of groups and individuals in the simulated signaling environment). This script performs our simulation experiments over 10 iterations of each treatment combination.
  
  2. `Simulation_Experiments_HPCC.Rmd`: This RMarkdown file holds updates to `Simulation_Experiments.Rmd` that were made to perform 1000 iterations for each treatment combination on a high-performance computing cluster (HPCC). We did not produce an HTML file for this Rmd script because the code for the simulation experiment is overall the same aside from the number of iterations and resulting file names.
  
  3. `Simulation_Experiments_HPCC_additional.Rmd`: This script is a copy of `Simulation_Experiments_HPCC.Rmd` that was made in order to re-run specific treatment combinations on the supercomputer. We also did not knit an HTML file for this Rmd script.
  
  4. `AccuracyPlots_EffectSize.Rmd`: We plotted the accuracy results from simulation experiments and calculated effect sizes for statistical inference about identity information detection performance. In this script we also made a figure of the accuracy value distributions and effect size results.
  
  5. `AccuracyPlots_EffectSize_HPCC.Rmd`: This script is a copy of `AccuracyPlots_EffectSize.Rmd` that makes the same figures, but from the results obtained across 1000 iterations of our simulation experiment. This script also contains detailed notes on which treatment combinations had to be removed from the original results from our simulation experiment, and then rerun on the supercomputer before producing final figures.  
